...
目前我已经完成 OV5640 驱动、DCMI + DMA 图像采集、LCD 实时显示、串口图像导出、图像参数调试、双缓冲、基础图像处理和 UART CLI 在线调参。
...
我实现了一个简单的 UART CLI，可以在不重新编译程序的情况下切换图像处理模式和二值化阈值。
...
我准备将当前裸机程序逐步迁移到 FreeRTOS，计划划分以下任务：
