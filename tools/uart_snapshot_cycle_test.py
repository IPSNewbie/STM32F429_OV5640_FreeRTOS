...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="多轮验证 SNAPSHOT PREPARE/RESTORE 与软件保护状态。"
    )
    ...
    parser.add_argument(
        "--tag", default="stage11_b7_snapshot_cycle", help="输出文件标签"
    )
...
def sanitize_tag(tag):
    ...
    return safe_tag or "stage11_b7_snapshot_cycle"
...
def write_summary(summary_path, args, stats, final_pass):
    """保存测试参数、统计、连续性和最终结论。"""
    lines = [
        "Stage 11B-7 SNAPSHOT Stop/Restore 循环测试汇总",
        "",
        f"port={args.port}",
        f"baud={args.baud}",
...
def main():
    ...
    prefix = f"snapshot_cycle_{args.tag}_{timestamp}"
    ...
    try:
        for cycle_index in range(1, args.cycles + 1):
            ...
            try:
                ...
                restore_ok = (
                    "SNAPSHOT RESTORE" in response
                    and "RESTORE_DEFERRED" in response
                )
                if not restore_ok:
                    errors.append("RESTORE 响应缺少预期状态")
            except Exception as error:
