...
# -*- coding: utf-8 -*-
"""
Stage 11C-5F：OV5640 PWDN 物理隔离自动测试脚本

前提：
1. 固件已实现 SD CAMOFF / SD CAMON / SD CAMPOWER。
2. SD CAMOFF：在 SD TAKEOVER ENTER 之后、SD INIT 之前，拉高 OV5640 PWDN，使摄像头进入 power-down。
3. 本脚本不写 SD 卡，不接 FATFS，只测试：
   - 摄像头 PWDN 隔离后，PC9/SDIO_D1 是否恢复高电平
   - HAL_SD_Init 是否 PASS
   - SD READTEST 0 / 2048 是否仍 DATA_CRC_FAIL
   - CAMON + EXIT + RESTORE 后图像是否恢复
"""
...
def main() -> int:
    """解析参数并执行完整测试流程。"""
    p = argparse.ArgumentParser(description="Stage 11C-5F OV5640 PWDN isolation diagnostic")
    ...
    tag = now_tag()
    log_path = out_dir / ("sd_cam_pwdn_diag_%s_log.txt" % tag)
    summary_path = out_dir / ("sd_cam_pwdn_diag_%s_summary.txt" % tag)
    ...
    try:
        ...
        add_line_summary(summary, "LINE_AFTER_CAMOFF_BEFORE_INIT", line)
