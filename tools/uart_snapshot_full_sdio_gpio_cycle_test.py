...
CSV_FIELDS = [
    "cycle",
    "prepare_result",
    "full_sdio_enter_result",
    "sd_init_deferred_result",
    "dump_block_result",
    "binary_block_result",
    "full_sdio_exit_result",
    "restore_result",
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="多轮验证完整 SDIO GPIO AF12 切换、退出和图像恢复闭环。"
    )
    ...
    parser.add_argument(
        "--tag",
        default="stage11_c2_full_sdio_gpio_cycle",
        help="输出文件标签",
    )
...
def sanitize_tag(tag):
    ...
    return safe_tag or "stage11_c2_full_sdio_gpio_cycle"
...
def read_for_duration(ser, timeout_seconds):
    """在固定时间内收集数据，用于确认 guard 下没有合法图像帧。"""
    data = bytearray()
    deadline = time.monotonic() + timeout_seconds
...
def validate_image_frame(frame):
    """校验 OV56RGB5 头、格式、尺寸、payload 长度和 CRC。"""
...
def write_summary(summary_path, args, stats, final_pass):
    """保存测试参数、统计、连续性和最终结论。"""
    lines = [
        "Stage 11C-2 完整 SDIO GPIO AF12 切换闭环循环测试汇总",
        "",
        f"port={args.port}",
        f"baud={args.baud}",
        ...
        f"full_sdio_enter_ok_count={stats['full_sdio_enter_ok_count']}",
        f"sd_init_deferred_ok_count={stats['sd_init_deferred_ok_count']}",
        f"text_dump_block_ok_count={stats['text_dump_block_ok_count']}",
        f"binary_block_ok_count={stats['binary_block_ok_count']}",
        f"full_sdio_exit_ok_count={stats['full_sdio_exit_ok_count']}",
        ...
        "说明：SD TAKEOVER ENTER 后 PC8/PC9/PC10/PC11/PC12/PD2 应完整切换到 SDIO AF12。",
        "说明：SD INIT 必须保持 NEED_TAKEOVER，不能真实初始化 SDIO/FATFS。",
        "说明：SD TAKEOVER EXIT 后完整 SDIO GPIO 应退出 AF12，PC8/PC9/PC11 应恢复为 DCMI AF13。",
...
def main():
    ...
    prefix = f"full_sdio_gpio_cycle_{args.tag}_{timestamp}"
    ...
    rows = []
    prepare_ok_count = 0
    full_sdio_enter_ok_count = 0
    sd_init_deferred_ok_count = 0
    text_dump_block_ok_count = 0
    binary_block_ok_count = 0
    full_sdio_exit_ok_count = 0
    ...
    try:
        for cycle_index in range(1, args.cycles + 1):
            ...
            try:
                ...
                prepare_ok = "DCMI stop OK" in response
                if not prepare_ok:
            ...
            full_sdio_enter_ok = False
            try:
                response = send_text_command(
                    ser, "SD TAKEOVER ENTER", args.guard_timeout
                )
                full_sdio_enter_ok = all(
                    keyword in response
                    for keyword in (
                        "full SDIO GPIO switched to AF12",
                        "sdio_full_gpio_af12_selected=1",
                        "sdio_full_gpio_switch_success_count=",
                    )
                )
                if "blocked, run SNAPSHOT PREPARE first" in response:
                    errors.append("FULL SDIO ENTER 被前置条件阻止")
                    full_sdio_enter_ok = False
                elif not full_sdio_enter_ok:
                    errors.append("FULL SDIO ENTER 未确认六引脚切换和状态字段")
            except Exception as error:
                errors.append(f"FULL SDIO ENTER 异常：{error}")
            if full_sdio_enter_ok:
                full_sdio_enter_ok_count += 1

            sd_init_deferred_ok = False
            try:
                response = send_text_command(ser, "SD INIT", args.guard_timeout)
                sd_init_deferred_ok = all(
                    keyword in response
                    for keyword in (
                        "NEED_TAKEOVER",
                        "is_initialized=0",
                ...
                if not sd_init_deferred_ok:
                    errors.append("SD INIT 未保持 NEED_TAKEOVER 和未初始化状态")
            except Exception as error:
                errors.append(f"SD INIT 异常：{error}")
            if sd_init_deferred_ok:
                sd_init_deferred_ok_count += 1
            ...
            full_sdio_exit_ok = False
            try:
                response = send_text_command(
                    ser, "SD TAKEOVER EXIT", args.guard_timeout
                )
                full_sdio_exit_ok = all(
                    keyword in response
                    for keyword in (
                ...
                if not full_sdio_exit_ok:
                    errors.append("FULL SDIO EXIT 未确认退出 AF12 并恢复 DCMI AF13")
            except Exception as error:
                errors.append(f"FULL SDIO EXIT 异常：{error}")
            if full_sdio_exit_ok:
                full_sdio_exit_ok_count += 1
            ...
            print(
                ...
                f"FULL_SDIO_ENTER={'PASS' if full_sdio_enter_ok else 'FAIL'} "
                f"SD_INIT_DEFERRED={'PASS' if sd_init_deferred_ok else 'FAIL'} "
                f"DUMP_BLOCK={'PASS' if dump_block_ok else 'FAIL'} "
                f"BINARY_BLOCK={'PASS' if binary_block_ok else 'FAIL'} "
                f"FULL_SDIO_EXIT={'PASS' if full_sdio_exit_ok else 'FAIL'} "
            ...
            rows.append(
                {
                    ...
                    "full_sdio_enter_result": (
                        "PASS" if full_sdio_enter_ok else "FAIL"
                    ),
                    "sd_init_deferred_result": (
                        "PASS" if sd_init_deferred_ok else "FAIL"
                    ),
                    "dump_block_result": "PASS" if dump_block_ok else "FAIL",
                    "binary_block_result": "PASS" if binary_block_ok else "FAIL",
                    "full_sdio_exit_result": (
                        "PASS" if full_sdio_exit_ok else "FAIL"
                    ),
    ...
    final_pass = (
        ...
        and full_sdio_enter_ok_count == args.cycles
        and sd_init_deferred_ok_count == args.cycles
        and text_dump_block_ok_count == args.cycles
        and binary_block_ok_count == args.cycles
        and full_sdio_exit_ok_count == args.cycles
    ...
    stats = {
        ...
        "full_sdio_enter_ok_count": full_sdio_enter_ok_count,
        "sd_init_deferred_ok_count": sd_init_deferred_ok_count,
        "text_dump_block_ok_count": text_dump_block_ok_count,
        "binary_block_ok_count": binary_block_ok_count,
        "full_sdio_exit_ok_count": full_sdio_exit_ok_count,
