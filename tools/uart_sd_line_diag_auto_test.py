...
# -*- coding: utf-8 -*-
"""
Stage 11C-5E：SDIO 数据路径 / 硬件线状态自动诊断脚本

前提：固件已实现 SD LINESTATE。
本脚本不写 SD 卡，不接 FATFS，只做串口 CLI、读块诊断、退出恢复和图像链路验证。
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 11C-5E SDIO line-state auto diagnostic")
    ...
    parser.add_argument("--command-gap", type=float, default=1.0)
    parser.add_argument("--after-prepare-delay", type=float, default=3.0)
    parser.add_argument("--after-enter-delay", type=float, default=5.0)
    ...
    tag = now_tag()
    log_path = out_dir / ("sd_line_diag_auto_%s_log.txt" % tag)
    summary_path = out_dir / ("sd_line_diag_auto_%s_summary.txt" % tag)
    ...
    try:
        ...
        add_line_summary(summary, "LINE_BEFORE_INIT", line)
