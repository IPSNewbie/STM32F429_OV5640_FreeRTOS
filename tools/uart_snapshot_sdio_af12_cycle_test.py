...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="多轮验证 SDIO AF12 切换、退出和图像恢复闭环。"
    )
    ...
    parser.add_argument(
        "--tag",
        default="stage11_b12_sdio_af12_cycle",
        help="输出文件标签",
    )
...
def sanitize_tag(tag):
    ...
    return safe_tag or "stage11_b12_sdio_af12_cycle"
...
def write_summary(summary_path, args, stats, final_pass):
    """保存测试参数、统计、连续性和最终结论。"""
    lines = [
        "Stage 11B-12 SDIO AF12 切换闭环循环测试汇总",
        "",
        f"port={args.port}",
        f"baud={args.baud}",
        ...
        "说明：SD TAKEOVER ENTER 后 PC8/PC9/PC11 应切换到 SDIO AF12。",
        "说明：SD INIT 必须保持 NEED_TAKEOVER，不能真实初始化 SDIO/FATFS。",
        "说明：SD TAKEOVER EXIT 后 PC8/PC9/PC11 应从 SDIO AF12 恢复为 DCMI AF13。",
...
def main():
    ...
    prefix = f"sdio_af12_cycle_{args.tag}_{timestamp}"
    ...
    try:
        for cycle_index in range(1, args.cycles + 1):
            ...
            try:
                ...
                takeover_enter_ok = all(
                    keyword in response
                    for keyword in (
                        "conflict pins switched to SDIO AF12",
                        "sdio_af12_selected=1",
                        "sdio_af12_switch_success_count=",
                    )
                )
                if "blocked, run SNAPSHOT PREPARE first" in response:
                    errors.append("AF12 ENTER 被前置条件阻止")
                    takeover_enter_ok = False
                elif not takeover_enter_ok:
                    errors.append("AF12 ENTER 未确认引脚切换和状态字段")
            except Exception as error:
                errors.append(f"AF12 ENTER 异常：{error}")
            ...
            try:
                ...
                takeover_exit_ok = all(
                    keyword in response
                    for keyword in (
                        "conflict pins restored to DCMI AF13 from SDIO AF12",
                ...
                if not takeover_exit_ok:
                    errors.append("AF12 EXIT 未确认退出 AF12 并恢复 DCMI AF13")
            except Exception as error:
                errors.append(f"AF12 EXIT 异常：{error}")
            if takeover_exit_ok:
            ...
            print(
                ...
                f"AF12_ENTER={'PASS' if takeover_enter_ok else 'FAIL'} "
                ...
                f"AF12_EXIT={'PASS' if takeover_exit_ok else 'FAIL'} "
