...
# -*- coding: utf-8 -*-
"""
Stage 11C-5E-2：SD LINESTATE after-init A/B 自动测试脚本

目的：
- 不在 SD INIT 前调用 SD LINESTATE。
- 先走 C5R 慢速初始化基线。
- SD INIT 成功后再采样 SD LINESTATE。
- 然后执行 READTEST 0 / 2048、退出、恢复、basic/repeat。

用于判断：
1. 如果本脚本 SD_INIT=PASS，而原 C5E 脚本 SD_INIT=FAIL：
   说明“SD INIT 前调用 SD LINESTATE”这一动作/时序与初始化失败强相关。
2. 如果本脚本 SD_INIT 仍 FAIL：
   说明 C5E 固件本身或当前板卡状态已经破坏 C5R 基线。
"""
...
class UartSession:
    ...
    def image_request(self, seq: int, timeout: float) -> ImageResult:
        ...
        self.log.append(
            "frame_id=%s, version=%s, fmt=%s, size=%sx%s, payload_len=%s, "
            "recv_crc=0x%08X, calc_crc=0x%08X, crc_ok=%s, elapsed_ms=%.2f"
...
def main() -> int:
    """解析参数并执行完整测试流程。"""
    p = argparse.ArgumentParser(description="Stage 11C-5E-2 after-init line-state diagnostic")
    ...
    p.add_argument("--after-restore-delay", type=float, default=1.0)
    args = p.parse_args()

    out_dir = Path(args.captures)
    out_dir.mkdir(parents=True, exist_ok=True)
    tag = now_tag()
    log_path = out_dir / ("sd_line_after_init_auto_%s_log.txt" % tag)
    summary_path = out_dir / ("sd_line_after_init_auto_%s_summary.txt" % tag)
    ...
    try:
        ...
        # 关键区别：这里不在 SD INIT 前调用 SD LINESTATE
        ...
        line = sess.cli("SD LINESTATE", max_time=8.0, after_delay=args.command_gap)
        add_line_summary(summary, "LINE_AFTER_EXIT", line)
